...
class Initer():
    ...
    def install_minicap(self):
        minicap = f"{Path.cwd()}/DaoMaiBase/daomai_minicap/{self.abi}/bin/minicap"
        if os.path.exists(minicap):
            # self._device.push(minicap, "/data/local/tmp/")
            self._device.sync.push(minicap, "/data/local/tmp/minicap", mode=0o644)


        minicapso = f"{Path.cwd()}/DaoMaiBase/daomai_minicap/{self.abi}/lib/android-{self.sdk}/minicap.so"
        if os.path.exists(minicapso):
            #self._device.push(minicapso, "/data/local/tmp/")
            self._device.sync.push(minicapso, "/data/local/tmp/minicap.so", mode=0o644)


    def install_minitouch(self):
        minicap = f"{Path.cwd()}/DaoMaiBase/daomai_minitouch/{self.abi}/bin/minitouch"
        if os.path.exists(minicap):
            #self._device.push(minicap, "/data/local/tmp/")
            self._device.sync.push(minicap, "/data/local/tmp/minitouch", mode=0o644)
    ...
    def install_atx(self):
        ...
        atx = f"{Path.cwd()}/DaoMaiBase/daomai_atx/{name}"

        if os.path.exists(atx):
            #self._device.push(atx, "/data/local/tmp/atx-agent")
            self._device.sync.push(atx, "/data/local/tmp/atx-agent", mode=0o777)

    def _install_uiautomator_apks2(self):
        self.shell("pm", "uninstall", "com.github.uiautomator")
        self.shell("pm", "uninstall", "com.github.uiautomator.test")

        for name in ("1.apk", "2.apk"):
            apk_path = f"{Path.cwd()}/DaoMaiBase/daomai_apk/{name}"
            if os.path.exists(apk_path):
                target_path = "/data/local/tmp/" + name
                self.logger.debug("Install %s", name)
                #self._device.push(apk_path, target_path)
                self._device.sync.push(apk_path, target_path, mode=0o644)
                self.shell('pm', 'install', '-r', '-t', target_path)

    def setup_atx_agent2(self):
        # stop atx-agent first
        self.shell(self.atx_agent_path, "server", "--stop")
        if self.is_atx_agent_outdated():
            self.logger.info("Install atx-agent %s", __atx_agent_version__)
    ...
    def install(self):
        ...
        self.install_minitouch() #thay thế // self.push_url(self.minitouch_url)
