...
class _BaseClient(object):
    ...
    def _package_version(self, package_name: str) -> Optional[packaging.version.Version]:
        ...
        try:
            return "2.3.3" if "2.3.3" in m.match() else ""  #packaging.version.parse(m.group('name') if m else "")
        except:
            return "2.3.3"
...
class _Device(_BaseClient):
    ...
    def get_sdk(self):
        return self.shell(['getprop', 'ro.build.version.sdk']).output.replace("\n","").replace("\r","").replace("\t","")
